...
class ParentLoginView(APIView):
    """Parent login using Kids IC number and password"""
    ...
    def post(self, request):
        ...
        try:
            ...
            if user.check_password(password):
                # ✅ DELETE OLD TOKEN - ni je yang tambah
...
def coach_schools_list(request):
    if hasattr(request.user, 'profile') and request.user.profile.role not in ['super_admin', 'club_admin']:
        ...
        schools = School.objects.filter(is_active=True)
    return render(request, 'students/coach_schools.html', {'schools': schools})


@login_required
def coach_school_students(request, school_id):
